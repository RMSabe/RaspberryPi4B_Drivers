...
/*
 * GPCLK Command Structure (1 UINT):
 *
 * UINT BITS 31-24: CMD
 * UINT BITS 23-16: GPCLK
 * UINT BITS 15-0: ARGS
 */
...
static unsigned int gpclk_data_io = 0;

static int gpclk_is_reg_bit_active(unsigned int register_value, unsigned int reference_bit)
{
...
static int gpclk_is_busy(unsigned int n_clk)
{
...
static void gpclk_set_mash_level(unsigned int n_clk, unsigned int mash_level)
{
...
static unsigned int gpclk_get_mash_level(unsigned int n_clk)
{
...
static void gpclk_invert_output(unsigned int n_clk, unsigned int invert)
{
	invert &= 0x00000001;
	unsigned int mapping_pos = 0;
	while(gpclk_is_busy(n_clk));
...
static int gpclk_output_is_inverted(unsigned int n_clk)
{
	unsigned int mapping_pos = 0;
	switch(n_clk)
	{
...
static void gpclk_enable(unsigned int n_clk, unsigned int enable)
{
	enable &= 0x00000001;
	unsigned int mapping_pos = 0;
	switch(mapping_pos)
...
static int gpclk_is_enabled(unsigned int n_clk)
{
	unsigned int mapping_pos = 0;
	switch(mapping_pos)
	{
...
static void gpclk_set_clk_src(unsigned int n_clk, unsigned int clk_src)
{
	clk_src &= 0x00000007;
	unsigned int mapping_pos = 0;
	while(gpclk_is_busy(n_clk));
...
static unsigned int gpclk_get_clk_src(unsigned int n_clk)
{
...
static void gpclk_set_int_divider(unsigned int n_clk, unsigned int clkdiv)
{
	clkdiv &= 0x00000FFF;
	unsigned int mapping_pos = 0;
	while(gpclk_is_busy(n_clk));
...
static unsigned int gpclk_get_int_divider(unsigned int n_clk)
{
...
static void gpclk_set_frac_divider(unsigned int n_clk, unsigned int clkdiv)
{
	clkdiv &= 0x00000FFF;
	unsigned int mapping_pos = 0;
	while(gpclk_is_busy(n_clk));
...
static unsigned int gpclk_get_frac_divider(unsigned int n_clk)
{
	unsigned int clkdiv = 0;
	unsigned int mapping_pos = 0;
	switch(n_clk)
...
static void gpclk_set_kill_bit(unsigned int n_clk, unsigned int bit_value)
{
...
static int gpclk_get_kill_bit(unsigned int n_clk)
{
	unsigned int mapping_pos = 0;
	switch(n_clk)
	{
...
static void gpclk_reset(unsigned int n_clk)
{
...
ssize_t gpclk_mod_usrwrite(struct file *file, const char __user *user, size_t size, loff_t *offset)
{
	...
	unsigned int cmd = (gpclk_data_io >> 24);
	unsigned int clk = ((gpclk_data_io & 0x00FF0000) >> 16);
	unsigned int arg = (gpclk_data_io & 0x0000FFFF);
	...
	switch(cmd)
	{
		...
		case GPCLK_CMD_SET_ENABLE:
			gpclk_enable(clk, arg);
			break;

		case GPCLK_CMD_GET_ENABLE:
			gpclk_data_io = gpclk_is_enabled(clk);
			break;

		case GPCLK_CMD_SET_CLKSRC:
			gpclk_set_clk_src(clk, arg);
			break;

		case GPCLK_CMD_GET_CLKSRC:
			gpclk_data_io = gpclk_get_clk_src(clk);
			break;

		case GPCLK_CMD_SET_INVERT_OUTPUT:
			gpclk_invert_output(clk, arg);
			break;

		case GPCLK_CMD_GET_INVERT_OUTPUT:
			gpclk_data_io = gpclk_output_is_inverted(clk);
			break;

		case GPCLK_CMD_SET_MASH_LEVEL:
			gpclk_set_mash_level(clk, arg);
			break;

		case GPCLK_CMD_GET_MASH_LEVEL:
			gpclk_data_io = gpclk_get_mash_level(clk);
			break;

		case GPCLK_CMD_SET_IDIVIDER:
			gpclk_set_int_divider(clk, arg);
			break;

		case GPCLK_CMD_GET_IDIVIDER:
			gpclk_data_io = gpclk_get_int_divider(clk);
			break;

		case GPCLK_CMD_SET_FDIVIDER:
			gpclk_set_frac_divider(clk, arg);
			break;

		case GPCLK_CMD_GET_FDIVIDER:
			gpclk_data_io = gpclk_get_frac_divider(clk);
			break;

		case GPCLK_CMD_GET_IS_BUSY:
			gpclk_data_io = gpclk_is_busy(clk);
			break;

		case GPCLK_CMD_SET_KILL_BIT:
			gpclk_set_kill_bit(clk, arg);
			break;

		case GPCLK_CMD_GET_KILL_BIT:
			gpclk_data_io = gpclk_get_kill_bit(clk);
			break;
	}
